...
if [ "$START_NODES" = true ]; then
    python3 baseline_performance_test.py --samples $SAMPLES --output "$OUTPUT_FILE"
else
    python3 baseline_performance_test.py --samples $SAMPLES --output "$OUTPUT_FILE" --no-nodes
fi
